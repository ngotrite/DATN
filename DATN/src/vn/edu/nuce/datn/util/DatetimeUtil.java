...
public class DatetimeUtil
{
	...
	static public Object [] getPeriod_Of_Time(String Code, String year){
		//Code: T01, T02 ... T12: Thang
		//Code: Q01 ... Q04: Quy
		//Code: N01: 6 thang dau; N02: 6 thang cuoi; N03: Nam
		Timestamp fromDate = null;
		Timestamp toDate = null;
		String strDate = year + "-01-01 00:00:00.0";
		Timestamp dateBase = Timestamp.valueOf(strDate);
		int number = 0;
		String key = "";
		if (!Code.equals("") && !year.equals("") && !Code.equals("OTH")){
			key = Code.substring(0,1);
			number = Integer.parseInt(Code.substring(1));
			
			if(key.equals("T")) { //Thang
				number --;
				Calendar cl = Calendar.getInstance();
				cl.setTime(dateBase);
				cl.set(Calendar.MONTH, number);				
				fromDate = new Timestamp(cl.getTime().getTime());
				cl.set(Calendar.MONTH, number + 1);		
				cl.set(Calendar.DAY_OF_MONTH, 0);
				toDate = new Timestamp(cl.getTime().getTime());
			...
			if(key.equals("Q")) {// Quy
				if(number == 1){
					fromDate = dateBase;
					toDate = Timestamp.valueOf(year + "-03-31 00:00:00.0");
				}
				
				if(number == 2){
					fromDate = Timestamp.valueOf(year + "-04-01 00:00:00.0");
					toDate = Timestamp.valueOf(year + "-06-30 00:00:00.0");				
				}
				
				if(number == 3){
					fromDate = Timestamp.valueOf(year + "-07-01 00:00:00.0");
					toDate = Timestamp.valueOf(year + "-09-30 00:00:00.0");
				}
				
				if(number == 4){
					fromDate = Timestamp.valueOf(year + "-10-01 00:00:00.0");
					toDate = Timestamp.valueOf(year + "-12-31 00:00:00.0");
				}
			}
			
			if(key.equals("N") && number <= 2){ //Nua nam
				if(number == 1){
					fromDate = dateBase;
					toDate = Timestamp.valueOf(year + "-06-30 00:00:00.0");
				}
			...
			if(key.equals("N") && number == 3){ //Nua nam
				fromDate = dateBase;
				toDate = Timestamp.valueOf(year + "-12-31 00:00:00.0");
			}			
			
			if (fromDate != null && toDate != null) {
				Object [] data = {fromDate, toDate}; 
				return data;
			}
		}
	...
	public static String dateToString(Date value, String language) {
	...
	public static Timestamp parseTimestamp(String value, String format) {
		...
		if (value != null && value.length() > 0) {
			try {
				SimpleDateFormat df = new SimpleDateFormat(format);
				ts = new Timestamp(df.parse(value).getTime());
	...
	public static Timestamp parseTimestamp(String value) {
		...
		ts = parseTimestamp(value, format);
		if(ts == null) {
			format = "dd-MM-yyyy";
			ts = parseTimestamp(value, format);
		}		
